"""
FinBot — Advanced Multi-Agent Fintech Chatbot
=============================================
...
- ROUTER agent: classifies intent before planning (avoids Planner overload)
- ANOMALY agent: statistical + rule-based fraud/outlier detection
- Structured prompts with explicit JSON schema → fewer parse failures
...
SAFE_BUILTINS = {
    ...
    "isinstance": isinstance, "print": lambda *a, **k: None,  # silenced
}
...
def extract_json(raw: str, fallback: dict = None) -> dict:
    """
    Try 4 strategies to extract JSON from LLM output.
    Never raises — returns fallback dict on total failure.
    """
    if fallback is None:
        fallback = {}
    ...
    # Strategy 1: strip markdown fences, find outermost { }
    def try_parse(s: str) -> Optional[dict]:
        ...
        # Strategy 2: fix common Gemini issues
        #   - trailing commas before }  or ]
        #   - single quotes instead of double
        #   - unescaped newlines inside strings
        candidate = re.sub(r",\s*([}\]])", r"\1", candidate)    # trailing commas
        candidate = re.sub(r"(?<![\\])'", '"', candidate)       # single → double quotes
        candidate = re.sub(r'(?<!\\)\n', ' ', candidate)        # newlines in strings
        try:
            return json.loads(candidate)
        except Exception:
            pass

        # Strategy 3: extract key-value pairs manually for simple structures
        obj = {}
        for m in re.finditer(r'"(\w+)"\s*:\s*"([^"]*)"', candidate):
            obj[m.group(1)] = m.group(2)
        for m in re.finditer(r'"(\w+)"\s*:\s*([\d.]+)', candidate):
            try: obj[m.group(1)] = float(m.group(2))
    ...
    # Strategy 4: ask Gemini to repair it (only if text is long enough to be worth it)
    if len(text) > 30:
        try:
            repair_model = genai.GenerativeModel(
                model_name         = GEMINI_MODEL,
                generation_config  = genai.GenerationConfig(max_output_tokens=800, temperature=0),
            )
            repair_prompt = (
                "The following text should be valid JSON but has syntax errors. "
                "Return ONLY the corrected JSON object, nothing else:\n\n" + text[:2000]
            )
            repaired = repair_model.generate_content(repair_prompt).text.strip()
...
def call_gemini(model, message: str, history: list, retries: int = MAX_API_RETRIES) -> str:
    """
    ...
    for attempt in range(retries):
        try:
            ...
            if "429" in err or "quota" in err.lower():
                wait = max(wait, 15)
            print(f"[gemini retry {attempt+1}] {err} — waiting {wait}s")
            time.sleep(wait)
...
def result_to_str(result: Any, max_rows: int = 15) -> str:
    if isinstance(result, pd.DataFrame): return df_to_str(result, max_rows)
    if isinstance(result, pd.Series):    return df_to_str(result.reset_index(), max_rows)
...
# ─────────────────────────────────────────────────────────────────────────────
# SYSTEM PROMPTS  (all use double-brace {{ }} for literal JSON examples)
# ─────────────────────────────────────────────────────────────────────────────

ROUTER_SYSTEM = """You are a financial query ROUTER. Classify the user's question into one category.

Categories:
- analysis    : aggregations, groupby, sums, counts, averages, rankings
- filter      : user wants to slice/filter the data (contains: "only", "filter", "where", "show me X from Y")
- anomaly     : outliers, duplicates, suspicious, unusual, fraud detection
- forecast    : predict, trend, future, next month/quarter/year
...
PLANNER_SYSTEM = """You are the PLANNER agent for a financial analysis system.
...
Rules:
- chart type must be one of: bar, line, pie, scatter, none
...
- For outliers: Q1=df[col].quantile(0.25); Q3=df[col].quantile(0.75); IQR=Q3-Q1
...
INSIGHTS_SYSTEM = """You are a senior fintech analyst. Generate specific insights from data results.

Rules:
- Reference actual numbers from the result
- No generic advice — only data-backed observations
- Insights: patterns, concentrations, anomalies you see
- Tips: concrete actionable business decisions
...
KPI_SYSTEM = """You are the KPI agent. Compute 8 key financial metrics from dataframe `df`.
...
Prioritize: total spend/amount, average order value, unique suppliers/vendors, record count, top category spend, date range span.
Wrap risky code in try/except, default value to 0 on error.
...
Detection methods to apply (use those relevant to available columns):
1. IQR outliers on numeric columns (amount, quantity)
2. Duplicate detection (same PO number, same amount+supplier+date)
3. Round-number bias (amounts ending in 000 — potential fraud signal)
4. Weekend/holiday transactions (if date column exists)
5. Vendors with single transactions of unusually high value

Return a DataFrame with columns: anomaly_type, description, risk_level (High/Medium/Low), plus the original columns.
...
def compute_forecast(df: pd.DataFrame, date_col: str, value_col: str, periods: int = 6) -> Optional[dict]:
    try:
        ...
        for i in range(1, periods + 1):
            ...
            future_values.append(safe_val(float(model.predict([[len(monthly) + i - 1]])[0])))
        ...
        return {
            ...
            "slope_per_month": safe_val(slope),
...
class FinancialChatbot:
    ...
    def __init__(self, df: pd.DataFrame):
        ...
        self.conversation_ctx   = []   # high-level QA memory for context
        ...
        filter_ctx = {**ctx, "sample": json.dumps(sample_vals, default=str, ensure_ascii=False)}

        def make(system: str, tokens: int = 1200, temp: float = 0.05, fctx: bool = False):
            try:
                fmt = filter_ctx if fctx else ctx
                return genai.GenerativeModel(
                    model_name         = GEMINI_MODEL,
                    system_instruction = system.format(**fmt),
                    generation_config  = genai.GenerationConfig(
                        max_output_tokens   = tokens,
                        temperature         = temp,
                        response_mime_type  = "text/plain",  # prevent Gemini from adding extra wrappers
                    ),
                )
            except Exception as e:
        ...
        self.router_model   = make(ROUTER_SYSTEM,   tokens=200,  temp=0)
        self.planner_model  = make(PLANNER_SYSTEM,  tokens=800,  temp=0.05)
        self.executor_model = make(EXECUTOR_SYSTEM, tokens=1200, temp=0.05)
        self.critic_model   = make(CRITIC_SYSTEM,   tokens=500,  temp=0)
        self.insights_model = make(INSIGHTS_SYSTEM, tokens=700,  temp=0.3)
        self.filter_model   = make(FILTER_SYSTEM,   tokens=400,  temp=0,   fctx=True)
        self.kpi_model      = make(KPI_SYSTEM,      tokens=1200, temp=0.05)
        self.anomaly_model  = make(ANOMALY_SYSTEM,  tokens=1200, temp=0.05)
    ...
    def _chat(self, model, history: list, message: str) -> str:
        if model is None:
            raise RuntimeError("Model not initialized")
        text = call_gemini(model, message, history)
    ...
    def _run_code(self, code: str, prev_result: Any = None) -> tuple:
        """Execute code in restricted sandbox. Returns (result, error_str)."""
        local = {
            "df":          self.df,
            "pd":          pd,
            "np":          np,
    ...
    def _route(self, question: str) -> str:
        """Classify question intent. Returns intent string."""
        try:
            raw    = call_gemini(self.router_model, question, [])
    ...
    def _planner(self, question: str) -> dict:
        ...
        raw    = self._chat(self.planner_model, self.planner_hist, prompt)
    ...
    def _executor(self, action: str, prev_result: Any, plan: dict, fix_note: str = "") -> dict:
        ...
        raw      = self._chat(self.executor_model, self.executor_hist, prompt)
    ...
    def _critic(self, question: str, plan: dict, code: str, result: Any) -> dict:
        ...
        raw      = self._chat(self.critic_model, self.critic_hist, prompt)
    ...
    def _insights(self, question: str, result: Any) -> dict:
        ...
        try:
            raw = self._chat(self.insights_model, self.insights_hist, prompt)
    ...
    def apply_filter(self, user_text: str) -> dict:
        try:
            raw  = call_gemini(self.filter_model, f"Filter request: {user_text}", [])
        ...
        if expr in ("error", "", "null", "None"):
            return {"success": False, "error": "Could not parse filter — try rephrasing (e.g. 'only Germany' or 'amount > 50000')", "rows": len(self.df)}
    ...
    def compute_kpis(self) -> list:
        try:
            raw  = call_gemini(self.kpi_model, "Compute 8 KPIs.", [])
    ...
    def detect_anomalies(self) -> dict:
        """Run anomaly detection agent. Returns dict with results + summary."""
        try:
            raw  = call_gemini(self.anomaly_model, "Detect all anomalies in the dataset.", [])
            resp = extract_json(raw, {})
            code = resp.get("code", "")
            if code:
                result, err = self._run_code(code)
                if not err and isinstance(result, pd.DataFrame) and len(result) > 0:
                    summary = f"Found **{len(result):,} anomalies** across {result.get('anomaly_type', pd.Series()).nunique() if 'anomaly_type' in result.columns else '?'} categories."
                    return {
                        "found":   True,
                        "count":   len(result),
                        "table":   format_result_as_table(result),
                        "summary": summary,
                    }
                elif not err:
        ...
        # Fallback: simple IQR outlier detection
    ...
    def _fallback_anomalies(self) -> dict:
        ...
        if rows:
            ...
            return {"found": True, "count": len(combined), "table": format_result_as_table(combined), "summary": f"Found **{len(combined)}** statistical outliers."}
    ...
    def answer(self, user_message: str) -> dict:
        ...
        if intent == "anomaly":
            ...
            return {
                ...
                "growth_tips": ["Review high-risk transactions with your finance team", "Set up automated alerts for values exceeding 3× IQR", "Cross-check flagged vendors against approved vendor list"],
            }
        ...
        if intent == "forecast":
            ...
            trend_desc = (
                ...
                f"See the 📈 Forecast button in the top nav for the interactive chart."
            )
            return {"answer": trend_desc, "chart": None, "table": None, "code": None, "agent_log": agent_log,
                    "insights": [f"Trend direction: {fc['trend']}", f"R²={fc['r2']} — {'reliable' if fc['r2']>0.7 else 'approximate'} forecast"],
                    "growth_tips": ["Use forecast to negotiate annual contracts with top suppliers", "Budget next quarter based on projected spend trajectory", "Investigate causes if trend diverges from budget plan"]}

        # 1. PLAN
        try:
            plan = self._planner(user_message)
        except Exception as e:
            return {"answer": f"⚠️ Planning failed: {e}\n\nTry rephrasing your question.", "chart": None, "table": None, "code": None, "agent_log": agent_log, "insights": [], "growth_tips": []}
        ...
        for step in steps[:MAX_STEPS]:
            ...
            for attempt in range(MAX_CRITIC_RETRIES + 1):
                ...
                try:
                    ...
                    if reasoning: agent_log.append(f"  💭 {reasoning}")
                except Exception as e:
                    agent_log.append(f"  ❌ Executor failed: {e}")
                    break

                if not code:
                    agent_log.append("  ⚠️ No code generated")
                    break
                ...
                try:
                    ...
                    if issues: agent_log.append(f"  → Issues: {'; '.join(str(i) for i in issues)}")

                    if verdict == "FAIL" and fix_note and attempt < MAX_CRITIC_RETRIES:
                        agent_log.append(f"  → Requesting fix…")
                        continue
                except Exception as e:
                    agent_log.append(f"  ⚠️ Critic error: {e}")
                    review = {}
        ...
        if final_result is None:
            return {
                "answer":    "⚠️ Could not compute a result. Try rephrasing or break it into a simpler question.",
